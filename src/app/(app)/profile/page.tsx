...
import { scoreReasonLabels, scorePoints } from "@/lib/community-score";
...
export default async function ProfilePage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Reveal>
        <p className="text-xs uppercase tracking-[0.3em] text-gold-bright">Your profile</p>
      ...
      <Reveal delay={0.1} className="mt-8">
        <Card className="flex items-center gap-5">
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.15em] text-ink/45">DKU Life community score</p>
            <p className="mt-1 font-display text-4xl">{user.communityScore}</p>
            <p className="mt-1 text-sm text-ink/50">#{rank + 1} on campus</p>
          </div>
        </Card>
      </Reveal>

      <Reveal delay={0.15} className="mt-8">
        <h2 className="font-display text-xl">How you&apos;ve earned it</h2>
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
          {(Object.keys(scorePoints) as ScoreReason[]).map((reason) => (
            <div key={reason} className="rounded-2xl bg-paper-dim p-3">
              ...
              <p className="mt-0.5 text-xs text-ink/50">{scoreReasonLabels[reason]}</p>
            </div>
          ))}
        </div>
      </Reveal>
            ...
            {user.scoreEvents.map((e) => (
              <StaggerItem key={e.id}>
                <div className="flex items-center justify-between rounded-xl bg-paper-dim px-4 py-3 text-sm">
                  <span className="text-ink/75">{scoreReasonLabels[e.reason]}</span>
