      ...
      {canReview ? (
        alreadyReviewed ? (
          ...
          <div className="mt-6 space-y-4 rounded-2xl border border-ink/10 bg-paper-dim/60 p-4">
            ...
            <div>
              <span className="mb-1.5 block text-xs text-ink/60">{t("whichClassOptional")}</span>
              {selectedCourse ? (
                ...
                <div className="space-y-3 rounded-xl border border-ink/10 bg-paper p-3">
                  {!manualEntry ? (
                    ...
                    <>
                      ...
                      <Field label={t("courseCodeFieldLabel")} placeholder="COMPSCI 201" value={newCode} onChange={(e) => setNewCode(e.target.value)} />
                      <Field label={t("titleFieldLabel")} placeholder="Data Structures" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} />
                      <Button
                        type="button"
                        ...
                        className="w-full"
                      >
                        {linkingCourse ? t("adding") : t("addAndLinkCourse")}
                      </Button>
                    </>
                  )}
            ...
            <IconRatingPicker
              ...
              onChange={setGrading}
            />
            <IconRatingPicker icon={Star} activeClass="fill-gold-bright text-gold-bright" label={t("teachingPickerLabel")} value={teaching} onChange={setTeaching} />
            <IconRatingPicker icon={Smile} activeClass="text-sprout-deep" label={t("funPickerLabel")} value={fun} onChange={setFun} />
  ...
  return (
    <div>
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} type="button" onClick={() => onChange(n)} className="focus-ring p-0.5" aria-label={t("outOf5Aria", { n })}>
